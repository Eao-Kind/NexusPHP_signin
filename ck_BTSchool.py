...
class BTSchool:
    ...
    def my_init(self):
        ...
        self.cookie = get_cookie_by_json("BTSchoolCookie")
        self.headers = {
    ...
    def get_cookie_by_requests(self):
        ...
        sess.get("https://pt.btschool.club/userdetails.php?id=9")
        cookie = sess.cookies
    ...
    def sign(self):
        ...
        print(r.text)
        # 签到成功 、已经签到过了
        msg = re.match(".*今天签到您获得(.*？)点魔力值", r.text, re.S)
        if msg:  # 成功
            msg = msg.group(1)
        else:
            msg = "\n今天已经签到过了"
...
if __name__ == '__main__':
    j = 1
    data = get_data()
    _check_items = data.get("BTSchool", [])
    meg = BTSchool(check_items=_check_items).main()
    # send("HDSky", meg)
