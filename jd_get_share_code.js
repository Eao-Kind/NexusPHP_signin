...
const $ = new Env("获取互助码—日志版");
const JD_API_HOST = "https://api.m.jd.com/client.action";
let cookiesArr = [], cookie = '', message;
const jdCookieNode = $.isNode() ? require('./jdCookie.js') : '';
let ShareInfoFarm = '/farm '; // 农场
let ShareInfoPet = '/pet '; // 京东萌宠
let ShareInfoBean = '/bean '; // 种豆得豆
...
!(async () => {
  ...
  for (let i = 0; i < cookiesArr.length && i < 5; i++) {
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
  ...
  console.log('农场ShareInfoFarm:' + ShareInfoFarm);
  console.log('萌宠ShareInfoPet:' + ShareInfoPet);
  console.log('种豆ShareInfoBean:' + ShareInfoBean);
                ...
                if (data.data.bizCode === 0) {
                  ...
                  $.taskVos.map((item) => {
                    if (item.taskType === 14) {
					  ...
					  ShareInfoDdfactory += `${item.assistTaskDetailVo.taskToken}&`;
                    }
                  });
                }
              }
              ...
              if (safeGet(data)) {
                ...
                if (data["ret"] === 0) {
                  ...
                  if (data.factoryList && data.productionList) {
					...
					ShareInfoJxfactory += `${data.user.encryptPin}&`;
      ...
      try {
        if (err) {
          ...
          if (
			...
			ShareInfoPet += `${$.petInfo.shareCode}&`;
...
async function getJDFruit() {
  ...
  async function jdFruit() {
    ...
    if ($.farmInfo.farmUserPro) {
	  ...
	  ShareInfoFarm += `${$.farmInfo.farmUserPro.shareCode}&`;
    } else {
      /*console.log(
          ...
          try {
            if (safeGet(data)) {
              ...
              if (oc(() => data.data.result.taskVos)) {
				...
				ShareInfoHealth += `${oc(() => data.data.result.taskVos[0].assistTaskDetailVo.taskToken)}&`;
