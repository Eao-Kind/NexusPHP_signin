...
class HDSky:
    ...
    def my_init(self):
        ...
        self.cookie = get_cookie_by_json("HDSkyCookie")
        self.image_code_ajax = self.check_items.get("image_code_ajax")
        self.headers = {
    ...
    def sign(self):
        # 获取图片 需要发送的data——构造请求——签到——获取签到信息
        form_data = {'action': 'new'}
        res = requests.post(self.image_code_ajax, headers=self.headers, data=form_data)
        data = res.json()
        imagehash = data["code"]
        print("签到图片地址：" + self.imgUrl + imagehash)
        img = requests.get(self.imgUrl + imagehash).content
        res = identify(img)
        print("图片识别结果：" + res)
    ...
    def main(self):
        ...
        if not self.check():
            ...
            self.my_init()
            msg_all += msg
            self.main()  # 这里调用得不好，会出现完成签到后继续调用下面的,所有下面应该加入else
        else:
            msg = self.sign()
            print(msg)
            msg_all += msg
...
if __name__ == '__main__':
    j = 1
    data = get_data()
    _check_items = data.get("HDSky", [])
    meg = HDSky(check_items=_check_items).main()
    send("HDSky", meg)
